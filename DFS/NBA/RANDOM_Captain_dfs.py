...
data = xl.load_workbook("C:\\Users\\jhansen3\\OneDrive - KPMG\\Documents\\Python\\Gambling\\DFS\\NBA\\Data_Captain.xlsx")
c = data.worksheets[1]
flex = data.worksheets[2]
...
arrays = [[c_array, flex_array], [c_array_salary, flex_array_salary]]
...
actual_array = [[arrays[0][0], arrays[0][1], arrays[0][1], arrays[0][1], arrays[0][1], arrays[0][1], arrays[0][1]], [arrays[1][0], arrays[1][1], arrays[1][1], arrays[1][1], arrays[1][1], arrays[1][1]]]
...
for i in range(0, 1000000000):
    ...
    combo = [arrays[0][0][0], arrays[0][1][0], arrays[0][1][1], arrays[0][1][2], arrays[0][1][3], arrays[0][1][4]]
    ...
    if len(combo) == len(set(combo)):
        ...
        for j in range(0, 6):
            ...
            while combo[j] != datasheet.cell(k, 3).value:
                k = k+1
            if j == 0:
                salary = salary + 1.5*datasheet.cell(k, 6).value
                points = points + 1.5*datasheet.cell(k, 8).value
            else:
        ...
        """
        if check == 0:
            if points > top_100[99][1] and salary<75000:
