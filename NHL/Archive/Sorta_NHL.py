...
if difference == 0:
    ...
    for i in range(0, len(bets)):
        ...
        if bets[i] == "60 Min Winning Margin":
            ...
            for h in range(0, len(show_all)):
                time.sleep(5)
                action.click(on_element = show_all[h])
                action.perform()
                messy_teams = driver.find_elements(By.XPATH, "//*[contains(@data-automation-id, 'competition-event-participant')]")        
                for a in range(tracker, 7+tracker):
                    teams.append(messy_teams[a].text)
                    tracker = tracker + 5
                ...
                for a in range(tracker, 7+tracker):
                    print(messy_more_odds[a].text)
                    if messy_more_odds[a].text == "Suspended":
                        more_odds.append(0)
                    else:
                        more_odds.append(float(messy_more_odds[a].text))
        else:
            messy_teams = driver.find_elements(By.XPATH, "//*[contains(@data-automation-id, 'competition-event-participant')]")        
