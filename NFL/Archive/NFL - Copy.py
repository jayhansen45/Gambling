...
newLocation = "C:\\Users\\jayha\\Documents\\Gambling\\Automated\\NFL\\NFL Tracker.xlsx"
...
if difference == 0:
    ...
    for i in range(0, len(bets)):
        ...
        if bets[i] == "Total Points":
            ...
            j = 0
            for a in messy_total_points:
                    temp = a.text
                    temp2 = temp.split("(")
                    temp3 = temp2[1].split(")")
                    total_points.append(float(temp3[0]))
            for row in range(2, 17):
                sheet.cell(row+m, 6).value = total_points[row-1]
